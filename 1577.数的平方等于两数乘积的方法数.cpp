...
public:
    int numTriplets(vector<int>& nums1, vector<int>& nums2) {
        auto cal = [](vector<int>& n1, vector<int>& n2) -> int {
            int res = 0;
            for (long long x : n1) {
                long long target = x * x;
                int j = 0, k = n2.size() - 1;
                while (j < k) {
                    long long prod = 1LL * n2[j] * n2[k];
                    if (prod == target) {
                        // 【关键修正】：如果两端数字相等，说明中间全部相等
                        if (n2[j] == n2[k]) {
                            int count = k - j + 1;
                            res += count * (count - 1) / 2;
                            break; // 这一行处理完了，直接跳出 while
                        } else {
                            // 两端数字不等，分别统计两端重复的数量
                            int leftCnt = 1, rightCnt = 1;
                            while (j + 1 < k && n2[j] == n2[j+1]) { j++; leftCnt++; }
                            while (k - 1 > j && n2[k] == n2[k-1]) { k--; rightCnt++; }
